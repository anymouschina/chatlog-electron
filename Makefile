...
.PHONY: all clean lint tidy test build crossbuild upx macapp windowsgui
...
macapp: build
	@echo "🍎 Packaging Chatlog.app for macOS..."
	bash script/create_macos_app.sh

windowsgui:
	@echo "🪟 Building Windows GUI (no console) launcher..."
	GOOS=windows GOARCH=amd64 CGO_ENABLED=1 $(GO) build -trimpath $(LDFLAGS) -ldflags "-H=windowsgui" -o bin/chatlog_windows_gui.exe main.go
